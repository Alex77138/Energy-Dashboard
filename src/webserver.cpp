...
// ─── Buffer temps réel pour les graphiques (RAM, sans SD) ─────────────────────
...
<nav>
  ...
  <button class="tab" onclick="showTab('charts',this)">Graphiques</button>
...
<div id="pane-status">
  ...
  <p style="font-size:12px;color:var(--muted);margin-top:10px">Les informations systeme sont disponibles dans l'onglet <strong>Configuration</strong>.</p>
</div>
...
<label><span class="lbl">Type d'appareil</span>
  <select name="grid_device" onchange="updGrid()">
    ...
    <optgroup label="Shelly Gen1">
      <option value="1">Shelly EM Gen1 — 1 sonde</option>
      <option value="2">Shelly EM Gen1 — 2 sondes</option>
      <option value="3">Shelly 3EM — 1 phase</option>
      <option value="4">Shelly 3EM — 2 phases</option>
      <option value="5">Shelly 3EM — 3 phases</option>
    </optgroup>
...
<label><span class="lbl">Type d'appareil</span>
  <select name="solar_device" onchange="updSolar()">
    ...
    <optgroup label="Shelly Gen1">
      ...
      <option value="5">Shelly EM Gen1 — 1 sonde</option>
      <option value="6">Shelly EM Gen1 — 2 sondes</option>
      <option value="7">Shelly 3EM — 1 phase</option>
      <option value="8">Shelly 3EM — 2 phases</option>
      <option value="9">Shelly 3EM — 3 phases</option>
    </optgroup>
    <optgroup label="Shelly Gen2 / Gen3">
...
<label><span class="lbl">Puissance crete solaire (Wc) — active la jauge ecran</span>
...
<div class="info-box">
  Token Long-Lived Access Token genere dans HA : Profil → Securite → Jetons d'acces longue duree.<br>
...
<div class="info-box" style="margin-top:8px;font-size:12px">
  Topics publies : <code>{topic}/grid/power_w</code>, <code>solar/power_w</code>, <code>grid/energy_kwh</code>, <code>solar/energy_kwh</code>, <code>solar/dc_voltage</code>, <code>solar/limit_pct</code>, <code>system/rssi</code>
</div>
<div class="actions">
...
<!-- ══ Graphiques ══════════════════════════════════════════════════════════ -->
<div id="pane-charts" style="display:none">
<section>
<h3>PUISSANCES (W)</h3>
<div style="position:relative;height:220px"><canvas id="ch-power"></canvas></div>
</section>
<section>
<h3>AUTOCONSOMMATION &amp; AUTOSUFFISANCE (%)</h3>
<div class="info-box" style="font-size:12px">
  <strong>Autoconsommation</strong> : part de la production solaire consommee localement.<br>
  <strong>Autosuffisance</strong> : part de la consommation totale couverte par le solaire.
</div>
<div style="position:relative;height:220px"><canvas id="ch-rates"></canvas></div>
</section>
<div class="actions">
  <button class="btn btn-scan" onclick="loadCharts()">Actualiser</button>
  <span id="chart-status" style="font-size:13px;color:var(--muted);margin-left:10px"></span>
</div>
</div>
...
function fetchStatus(){
  fetch('/api/status').then(function(r){return r.json();}).then(function(d){
    ...
    document.getElementById('gk').textContent=d.grid.online?(d.grid.today_kwh.toFixed(2)+' kWh auj.'):'--';
    ...
    document.getElementById('sk').textContent=d.solar.online?(d.solar.today_kwh.toFixed(2)+' kWh auj.'):'--';
    ...
    if(d.ap_active){apEl.style.display='';document.getElementById('i-ap').textContent='DashEnergy-Config — '+d.ap_ip;}
    else{apEl.style.display='none';}

    // Jauge solaire uniquement
...
function startRefresh(){fetchStatus();if(!tid)tid=setInterval(fetchStatus,3000);}
...
function saveCfg(e){
  ...
  // HA fields (may be empty strings — that's fine)
...
/* ── Graphiques ── */
var chPower=null, chRates=null;
function fmtTs(ts){
  if(typeof ts==='string'&&ts[0]==='U') return 'U+'+ts.slice(1)+'s';
  var d=new Date(ts*1000);
  return d.getHours().toString().padStart(2,'0')+':'+d.getMinutes().toString().padStart(2,'0');
}
function computeRates(pts){
  return pts.map(function(d){
    var sp=d.sp||0, gp=d.gp||0;
    var self=sp+Math.min(0,gp); if(self<0)self=0;
    var load=sp+gp;
    return {
      ac: sp>0?Math.min(100,self/sp*100):null,
      as: load>0?Math.min(100,self/load*100):(sp>0?100:null)
    };
  });
}
function buildCharts(data){
  var labels=data.map(function(d){return fmtTs(d.ts);});
  var gp=data.map(function(d){return d.gp||0;});
  var sp=data.map(function(d){return d.sp||0;});
  var rates=computeRates(data);
  var ac=rates.map(function(r){return r.ac;});
  var as_=rates.map(function(r){return r.as;});
  var opts={animation:false,plugins:{legend:{labels:{color:'#e6edf3'}}},scales:{x:{ticks:{color:'#8b949e',maxTicksLimit:10},grid:{color:'#30363d'}},y:{ticks:{color:'#8b949e'},grid:{color:'#30363d'}}}};
  if(chPower) chPower.destroy();
  chPower=new Chart(document.getElementById('ch-power'),{type:'line',data:{labels:labels,datasets:[
    {label:'Consommation (W)',data:gp,borderColor:'#58a6ff',backgroundColor:'rgba(88,166,255,.1)',fill:true,tension:.3,pointRadius:0,borderWidth:2},
    {label:'Production (W)',data:sp,borderColor:'#f4a429',backgroundColor:'rgba(244,164,41,.1)',fill:true,tension:.3,pointRadius:0,borderWidth:2}
  ]},options:JSON.parse(JSON.stringify(opts))});
  if(chRates) chRates.destroy();
  var opts2=JSON.parse(JSON.stringify(opts));
  opts2.scales.y.min=0;opts2.scales.y.max=100;
  chRates=new Chart(document.getElementById('ch-rates'),{type:'line',data:{labels:labels,datasets:[
    {label:'Autoconsommation (%)',data:ac,borderColor:'#3fb950',backgroundColor:'rgba(63,185,80,.1)',fill:true,tension:.3,pointRadius:0,borderWidth:2,spanGaps:true},
    {label:'Autosuffisance (%)',data:as_,borderColor:'#f4a429',backgroundColor:'rgba(244,164,41,.1)',fill:true,tension:.3,pointRadius:0,borderWidth:2,spanGaps:true}
  ]},options:opts2});
}
function loadCharts(){
  var st=document.getElementById('chart-status');
  st.textContent='Chargement...';
  // Charger Chart.js si pas encore present
  if(typeof Chart==='undefined'){
    var s=document.createElement('script');
    s.src='https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.min.js';
    s.onload=function(){fetchChartData();};
    s.onerror=function(){st.textContent='Chart.js non disponible (connexion internet requise)';};
...
function fetchChartData(){
  ...
  fetch('/api/realtime').then(function(r){return r.json();}).then(function(data){
    if(!data||!data.length){
      st.textContent='Pas encore de donnees (attendre quelques cycles de polling)';return;
    }
    buildCharts(data);
    st.textContent=data.length+' points — '+fmtTs(data[0].ts)+' → '+fmtTs(data[data.length-1].ts);
  }).catch(function(){st.textContent='Erreur';});
}
...
static void handle_realtime() {
    // Copie thread-safe du ring buffer
    RTPoint snap[90];
    int head, count;
    taskENTER_CRITICAL(&rt_mux);
    ...
    // Reconstruire l'ordre chronologique
    static char buf[8192];
    size_t pos = 0;
    buf[pos++] = '[';
    int start = (count < 90) ? 0 : head;  // le plus ancien
